...
def plot_images(images, de, targets, paths=None, fname='images.jpg', names=None, max_size=1920, max_subplots=16):
    ...
    if isinstance(de, torch.Tensor):
        de = de.cpu().float().numpy()
    if np.max(images[0]) <= 1:
        images *= 255.0  # de-normalise (optional)
    if np.max(de[0]) <= 1:
        de *= 256.0  # de-normalise (optional)
    bs, _, h, w = images.shape  # batch size, _, height, width
    bs_de, h_de, w_de = de.shape  # batch size, _, height, width
    ...
    for i, im in enumerate(images):
        ...
        alpha = 0.5  # transparency factor, you can modify it
        blended_array = (1 - alpha) * im + alpha * (np.stack([de[i]] * 3, axis=-1)*256)
        mosaic[y:y + h, x:x + w, :] = blended_array[:, :, :3]
